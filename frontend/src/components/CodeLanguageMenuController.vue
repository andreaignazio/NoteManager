...
const activeMenuEl = computed(() => (menuOpen.value ? (menuRef.value?.el?.value ?? null) : null))
...
const items = computed(() => LANGS.map(l => ({
  type: 'item',
  id: `lang:${l.id}`,
  label: l.label,
...
async function onAction({ id, payload }) {
  ...
  if (id.startsWith('lang:')) {
    await blocksStore.updateBlockContent(String(props.blockId), { language: payload.language })
    close()
    return
  }
  close()
