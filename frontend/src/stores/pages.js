...
export const usePagesStore = defineStore("pagesStore", {
  ...
  actions: {
    ...
    async addPageAfterId(prevPageId) {
      try {
        ...
        const nextPos = nextId
          ? (this.pagesById[nextId]?.position ?? null)
          : null;
    ...
    async movePageInside(pageId, newParentId) {
      ...
      const lastSiblingId = newSiblingsIds.at(-1)
        ? newSiblingsIds.at(-1)
        : null;

      const lastSiblingPos = lastSiblingId
        ? this.pagesById[lastSiblingId].position
        : null;
    ...
    async movePageToParentAtIndex(pageId, newParentId, insertIndex) {
      ...
      const prevPos = prevId
        ? (this.pagesById[prevId]?.position ?? null)
        : null;
      const nextPos = nextId
        ? (this.pagesById[nextId]?.position ?? null)
        : null;
    ...
    async duplicatePageDeep(sourcePageId) {
      ...
      const nextPos = nextId
        ? (this.pagesById[String(nextId)]?.position ?? null)
        : null;
    ...
    async movePageToParentAppend(pageId, newParentId) {
      ...
      const lastPos = lastId
        ? (this.pagesById[lastId]?.position ?? null)
        : null;
      const newPos = posBetween(lastPos, null);
    ...
    updatePageLocationOptimistic(pageId, { newParentId, newPosition }) {
      ...
      if (this.childrenByParentId[oldParentKey]) {
        this.childrenByParentId[oldParentKey] = this.childrenByParentId[
          oldParentKey
        ].filter((id) => id !== pageId);
      }
    ...
    collectDescendantsIds(pageId, childrenByParentId) {
      const out = new Set();
