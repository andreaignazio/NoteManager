...
export interface OverlayBinding {
  ...
  // commands
  requestClose: (reason: 'escape'|'outside'|'replace'|'programmatic') => void
...
function resolveScope(v: OverlayInteractionScope | undefined): 'local' | 'global' {
  if (!v) return 'local'
...
export const useOverlayStore = defineStore('overlay', {
  ...
  actions: {
    ...
    sync(id: string) {
      ...
      if (open && !mounted) {
        ...
        const granted = this.request({
          ...
          interactionScope: b.interactionScope ? resolveScope(b.interactionScope) : 'local',
        })
        if (!granted) {
    ...
    _pushOrUpdate(incoming: OverlayEntry) {
      ...
      if (idx !== -1) {
        const prev = this.stack[idx]
        this.stack[idx] = { ...prev, ...incoming, _prevActive: prev._prevActive, createdAt: prev.createdAt }
      } else {
