...
import {
  ...
  createTextareaAutoResizer,
} from "@/composables/useAutoResizeTextarea";
...
const { isFocused, hasAnyFocus, onFocus, onBlur } = useBlockFocus(
  props.block.id,
);

const { saveRich, saveCode } = useBlockPersistence(props.block.id, {
  delay: 500,
});
...
const { handleSplitAndCreate } = useBlockSplit(
  editor,
  props.pageId,
  props.block.id,
);
...
//=== LIST BLOCKS ===
const olNumber = computed(() =>
  blocksStore.getOlNumber(props.pageId, props.block.id),
);
...
const displayValue = computed(() =>
  isEmpty.value ? "" : localTextContent.value,
);
...
/*const fontCss = computed(() => {
  const fontId = props.block?.props?.style?.font ?? "default";
  const fontCssVar = fontCssVar(fontId);
  console.log("fontCssVar", fontCssVar);
  return fontCssVar;
});*/
const fontCss = computed(() => {
  const fontId = props.block?.props?.style?.font ?? "default";
  return fontCssVar(fontId);
});
</script>
...
<template>
  ...
  >
    ...
    <div v-else class="tiptap-wrapper" :class="typeClass">
      <div class="block-style" :style="{ fontFamily: `${fontCss}` }">
        <editor-content :editor="editor" />
      </div>
    </div>
  </div>
...
<style scoped>
.block-style[data-font="default"] {
  font-family: var(--font-default);
}
.block-style[data-font="serif"] {
  font-family: var(--font-serif);
}
.block-style[data-font="mono"] {
  font-family: var(--font-mono);
}
.block-style[data-font="soft"] {
