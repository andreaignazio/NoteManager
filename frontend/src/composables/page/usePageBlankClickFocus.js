...
export function usePageBlankClickFocus(pageId, { getLocalTree }) {
  ...
  async function onPagePointerDown(e) {
    ...
    if (!pid) return
    if (overlay.hasAny && overlay.top?.options?.stopPointerOutside !== false) return
    ...
    if (blockItem) {
      ...
      if (id) {
        e.preventDefault()
        blocksStore.requestFocus(String(id), -1)
      }
      return
    }
    ...
    if (hit.mode === 'nearest') {
      e.preventDefault()
      await nextTick()
      blocksStore.requestFocus(String(hit.id), -1)
      return
    }
  ...
  function closestVisibleBlockByY(clientY) {
    ...
    if (!best) return null
    if (clientY > bestBottom) return { mode: 'below', lastVisibleId: bestIdByBottom, lastVisibleBottom: bestBottom }
  ...
  async function ensureFirstEmptyBlockAndFocus() {
    ...
    if (emptyId) {
      await nextTick()
      blocksStore.requestFocus(emptyId, 0)
      return
    }
    ...
    const newId = await blocksStore.addNewBlock(pid, {
      type: 'p',
      content: { text: '' },
      parent: null,
    }, lastId)
    ...
    await nextTick()
    if (newId) blocksStore.requestFocus(String(newId), 0)
  }

  function lastRootId(pid) {
    const roots = (blocksStore.childrenByParentId?.[pid]?.root ?? []).map(String)
  ...
  async function focusOrCreateAtEndRoot(pid) {
    ...
    if (lastId) {
      ...
      if (isEmptyParagraph(last)) {
        await nextTick()
        blocksStore.requestFocus(String(lastId), -1)
        return
      }
    }
    ...
    await nextTick()
    if (newId) blocksStore.requestFocus(String(newId), -1)
  }
  ...
  return { onPagePointerDown, registerRowEl, ensureFirstEmptyBlockAndFocus, lastEmptyRootId }
}
